                ...
                <div class="col-lg-8 mx-auto shadow p-2 p-sm-3">
                    ...
                    <div class="recommend">
                        <layout-card-list-wrapper classBreakPoint="1" classSmBreakPoint="2" classMdBreakPoint="2"
                            ...
                            <layout-card-list name="work" v-for="(item, index) in workItems" :id="item.id" :key="index"
                                :date="item.date" :image="item.image" :image-url="item.image.url" :body="item.body" :title="item.title"
                                :isShadow="false" classInBreakPoint="1" classSmInBreakPoint="1" classMdInBreakPoint="1"
    ...
    async asyncData({ $config, params, error }) {
        try {
            const { data } = await axios.get(
                `${$config.apiUrl}/work/${params.id}`,
                {
                    headers: { 'X-API-KEY': $config.apiKey },
                }
            )
            const work = await axios.get(`${$config.apiUrl}/work?limit=3&filters=flag[equals]true`, {
                headers: { 'X-API-KEY': $config.apiKey },
            })
            return {
                item: data,
